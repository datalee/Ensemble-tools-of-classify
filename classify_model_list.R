classify_model_list <-
  function(container, model, s=0.01, ...) {
    ...
    if (pmatch("svm",class(model),nomatch=0) > 0){
      svm_results <- predict(model,as.matrix(container[1][[1]]), probability=TRUE, ...) #Extract Label
      ...
      if (pmatch("slda",class(model),nomatch=0) > 0){
        slda_results <- predict(model,data.frame(as.matrix(container[1][[1]])),...)
        ...
        if (pmatch("LogitBoost",class(model),nomatch=0) > 0) {
          lboost_results <- predict(model,xtest=as.matrix(container[1][[1]]),type="raw",...) #Probability
          ...
          if (pmatch("classbagg",class(model),nomatch=0) > 0) {
            bagging_results <- predict(model,newdata=data.frame(as.matrix(container[1][[1]])), type=c("prob"),...)
            ...
            if (pmatch("randomForest",class(model),nomatch=0) > 0){
              rf_results <- predict(model,newdata=as.matrix(container[1][[1]]),type="prob",...)
              ...
              if (pmatch("glmnet",class(model),nomatch=0) > 0){
                classification_matrix <- as(as.matrix.csc(container[1][[1]]),"dgCMatrix")
                ...
                if (pmatch("tree",class(model),nomatch=0) > 0){
                  tree_results <- predict(model,newdata=data.frame(as.matrix(container[1][[1]])), type="vector",...)
                  ...
                  if (pmatch("nnet",class(model),nomatch=0) > 0){
                    nnet_results <- predict(model,newdata=data.frame(as.matrix(container[1][[1]])),...) #probabilities
                    ...
                    if (pmatch("maxent",class(model),nomatch=0) > 0) {
                      maxent_results <- predict(model,container[1][[1]],...)
